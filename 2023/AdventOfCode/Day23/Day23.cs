...
namespace Day23
{
    internal class Day23
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine($"First sum = {FindLongestPathLength(map, start, end)}");
            ...
            Console.WriteLine($"Second sum = {FindLongestPathLength(map, start, end)}");
        }

        private static int FindLongestPathLength(char[,] map, (int Row, int Col) start, (int Row, int Col) end)
        {
            var rowLength = map.GetLength(0);
            var colLength = map.GetLength(1);
            ...
            while (queue.Count > 0)
            {
                ...
                {
                    case '.':
                        toCheck.Add((row - 1, col, new (previousSteps)));
                        toCheck.Add((row + 1, col, new(previousSteps)));
                        toCheck.Add((row, col - 1, new(previousSteps)));
                        toCheck.Add((row, col + 1, new(previousSteps)));
                        ...
                        break;
                    case '>':
                        queue.Enqueue((row, col + 1, new(previousSteps)));
                        break;
                    case 'v':
                        queue.Enqueue((row + 1, col, new(previousSteps)));
                        break;
                }
            }
